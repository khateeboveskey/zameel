 ...
 */
export function getAdaptiveColor(color: string, tone: number) {
  const colorScheme = useColorScheme();
  return `$${color}${tone}${capitalize(colorScheme)}`;
}
 ...
 */
export function getAdabtiveTWColor(color: string, toneInDark: number = 500) {
